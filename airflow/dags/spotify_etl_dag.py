"""
Spotify ETL Pipeline DAG
Production-ready Airflow DAG for automated Spotify data extraction, transformation, and loading.

This DAG replaces the manual orchestration from SpotifyETLPipeline class with 
separate Airflow tasks for better monitoring, error handling, and scheduling.
...
# For Airflow 3.0.6 standalone - use basic operators
try:
    from airflow.operators.python import PythonOperator
    from airflow.operators.bash import BashOperator
    from airflow.hooks.base import BaseHook
except ImportError:
    # Fallback for different Airflow versions
...
try:
    ...
    logger.warning("⚠️ DAG will run in fallback mode with basic functionality")
...
dag = DAG(
    ...
    description='🎵 Automated Spotify Data ETL Pipeline',
    schedule=SCHEDULE_INTERVAL,
    catchup=CATCHUP,
    max_active_runs=MAX_ACTIVE_RUNS,
    tags=['spotify', 'etl', 'data-pipeline', 'production'],
    doc_md=__doc__,
)
...
def transform_data(**context):
    ...
    try:
        ...
        if transformed_df.empty:
            logger.warning("⚠️ Transformation resulted in empty dataset")
...
def generate_pipeline_report(**context):
    """
    Task 6: Generate a comprehensive pipeline execution report
    """
    try:
...
# 1. Check prerequisites (database + spotify auth) in parallel
...
[check_database_task, check_spotify_task] >> extract_data_task

# Sequential ETL pipeline
extract_data_task >> transform_data_task >> load_data_task >> report_task
