    ...
    def load_artists(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            if has_detailed_info:
                ...
                # Convert genres from comma-separated string to PostgreSQL array format
                if 'genres' in artists_df.columns:
                    artists_df['genres'] = artists_df['genres'].apply(
                        lambda x: None if pd.isna(x) or x == '' else f"{{{x.replace(', ', ',')}}}"
                    )
                ...
                logger.info("ℹ️ Using basic artist information (no detailed data available)")
