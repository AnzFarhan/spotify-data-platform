...
Features: Batch loading, upserts, transaction management, error handling
"""
import pandas as pd
import psycopg2
...
class SpotifyDatabaseLoader:
    """Load transformed Spotify data into PostgreSQL database"""
    ...
    def _setup_database(self):
        ...
        try:
            self.engine = create_engine(self.connection_string, pool_size=10, max_overflow=20)
    ...
    def load_artists(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            if 'artist_id' not in available_columns:
            ...
            artists_df['genres'] = None  # Will be populated later
            artists_df['popularity'] = None
            artists_df['followers'] = None
            artists_df['created_at'] = datetime.utcnow()
    ...
    def load_albums(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            albums_df['created_at'] = datetime.utcnow()
    ...
    def load_tracks(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            tracks_df['created_at'] = datetime.utcnow()
    ...
    def load_audio_features(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            features_df['created_at'] = datetime.utcnow()
    ...
    def load_listening_history(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            history_df['created_at'] = datetime.utcnow()
            ...
            if new_records:
                ...
                new_df.to_sql('listening_history', self.engine, if_exists='append', index=False)
